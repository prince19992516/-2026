...
| 查看角色轮换与章节进度 | [章节分工表与轮换示例](协作管理/章节分工表.md) |
| 不认识目录或.gitkeep | [目录和占位文件说明](教程/08-目录与gitkeep说明.md) |
...
B/C报告必须注明被审完整commit。三组都可以使用AI；人工核验与AI辅助部分分别记录，不把AI输出冒充独立手算。每个实际小组按本章承担的角色交对应报告；至少完成3个不同章节的角色任务，分别有A编写、B验证、C批判的报告与证据。
...
| [06](教材/第06章-神经网络求解PDE/README.md) | 神经网络求解PDE | PINN与神经算子 | 残差为零但边界错误的解析反例 |
| [07](教材/第07章-可信的AI/README.md) | 可信的AI | 泛化、误差界与可信性 | 域内拟合与域外误差 |
...
本仓库依据2026文件夹原有的八章目录、README规划和环境草稿搭建；[原始规划](协作管理/原始目录规划.md)已保留。该文件夹未提供课程讲义、学时、学生名单或详细教学大纲，因此没有虚构这些信息。原始规划仅作历史存档，当前执行多小组轮换A/B/C角色的规则。

教学团队在章节分工表填写实际小组编号、轮次与日期，补充参考教材和评分规则，并确认[内容授权说明](版权与引用说明.md)。
